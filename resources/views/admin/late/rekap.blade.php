...
@section('content')
  ...
  <div class="card px-3 shadow p-2">
    <div class="card-body">
      <a href="{{ route('late.create') }}" class="btn btn-primary hover:bg-primary-800"> Tambah</a>
        ...
        <tbody>
          ...
          @foreach ($rekap as $item)
            <tr>
              ...
              <td>{{ $item['total'] }}</td>
              <td><a href="{{ route('late.detail', ['id' => $item->student_id]) }}" class="text-blue-500 hover:text-blue-700">Lihat</a></td>
              <td>
                @if ($item->total>= 3)
                  <a  class="btn btn-danger btn-sm text-white hover:bg-red-800 display-flex" href="{{ route('late.print',  ['id' => $item->student_id] )}}">Surat Pernyataan</a>
                @endif
              </td>
            </tr>
          @endforeach
